...
OptimizerEngine::OptimizerEngine(QObject *p):QObject(p){
    connect(&m_watcher,&QFutureWatcher<TextureResult>::finished,this,[this]{
        try{
            ...
            const QString materials=dir.filePath("original/"+src.completeBaseName()+"_MATERIAL_MAP.png");
            if(!r.materialMap.save(materials,"PNG",100))throw std::runtime_error("Не удалось сохранить карту материалов");
            ...
            m_materialUrl=QUrl::fromLocalFile(materials).toString();
            m_resultUrl=QUrl::fromLocalFile(out).toString();m_report=r.report+"\n\nСохранено: "+m_outputPath;m_status="Готово — файл сохранён";
            emit resultUrlChanged();emit referenceUrlChanged();emit materialUrlChanged();emit reportChanged();emit outputPathChanged();emit statusChanged();
...
void OptimizerEngine::load(const QString&value){if(m_busy)return;const QUrl u(value);m_sourceUrl=u.isLocalFile()?u.toString():QUrl::fromLocalFile(value).toString();m_resultUrl={};m_referenceUrl={};m_materialUrl={};m_outputPath={};m_report={};m_status="Текстура готова к анализу";emit sourceUrlChanged();emit resultUrlChanged();emit referenceUrlChanged();emit materialUrlChanged();emit outputPathChanged();emit reportChanged();emit statusChanged();}
