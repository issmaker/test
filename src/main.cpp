        ...
        if(startupTest){
            ...
            QTimer::singleShot(7600,view,[view]{
                view->page()->runJavaScript(QStringLiteral(R"JS((()=>{
                    ...
                    const stressSafe=Boolean(window.__AGR_STRESS_DONE__)&&Number(window.__AGR_WHEEL_COUNT__||0)>=40&&Number(window.__AGR_ZOOM_TARGET__||0)>=15.99&&Number(window.__AGR_ZOOM_BUTTON_SCALE__||0)===16&&Boolean(window.__AGR_FULL_AT_800__)&&Number(window.__AGR_FULL_DETAIL_VISIBLE_SCALE__||0)>=15.5;
                    ...
                    return `${mask}|${zoomLabel?.textContent}|${window.__AGR_WHEEL_COUNT__||0}|${window.__AGR_ZOOM_TARGET__||0}|${document.querySelectorAll('.smooth-compare').length}|${document.querySelectorAll('.smooth-compare canvas').length}|${window.__AGR_TEST_VERTICAL_Y__||0}|${window.__AGR_ZOOM_ANCHOR_ERROR__||0}|${window.__AGR_FULL_DETAIL_READY__||0}|${window.__AGR_FULL_AT_800__||false}|${window.__AGR_FULL_DETAIL_SCALE__||0}|${window.__AGR_FULL_DETAIL_VISIBLE_SCALE__||0}`;
