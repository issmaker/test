...
OptimizerEngine::OptimizerEngine(QObject *p):QObject(p){
    connect(&m_watcher,&QFutureWatcher<TextureResult>::finished,this,[this]{
        try{
            ...
            emit resultUrlChanged();emit reportChanged();emit outputPathChanged();emit statusChanged();
...
void OptimizerEngine::load(const QString&value){if(m_busy)return;const QUrl u(value);m_sourceUrl=u.isLocalFile()?u.toString():QUrl::fromLocalFile(value).toString();m_resultUrl={};m_outputPath={};m_report={};m_status="Текстура готова к анализу";emit sourceUrlChanged();emit resultUrlChanged();emit outputPathChanged();emit reportChanged();emit statusChanged();}
