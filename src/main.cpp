...
int main(int argc,char**argv){
    ...
    QObject::connect(view,&QWebEngineView::loadFinished,&window,[&](bool ok){
        ...
        if(startupTest){
                    ...
                    setTimeout(()=>{
                        ...
                        const promoteTo1600=()=>{
                            if(Number(window.__AGR_FULL_DETAIL_READY__||0)>0||Date.now()>=fullDeadline){
                                ...
                                const visibleDeadline=Date.now()+1600;
                                const stressAt1600=()=>{
                                    if(Number(window.__AGR_FULL_DETAIL_VISIBLE_TARGET__||0)>=15.99||Date.now()>=visibleDeadline){
                                    ...
                                    }else setTimeout(stressAt1600,50);
                                };
                                stressAt1600();
            ...
            QTimer::singleShot(11000,view,[view]{view->page()->runJavaScript(QStringLiteral("document.querySelector('.top-actions [data-action=\"settings\"]')?.click()"));});
            QTimer::singleShot(13200,view,[view]{
                view->page()->runJavaScript(QStringLiteral(R"JS((()=>{
                    ...
                    const stressSafe=Boolean(window.__AGR_STRESS_DONE__)&&Number(window.__AGR_WHEEL_COUNT__||0)>=40&&Number(window.__AGR_ZOOM_TARGET__||0)>=15.99&&Number(window.__AGR_ZOOM_BUTTON_SCALE__||0)===16&&Boolean(window.__AGR_FULL_AT_800__)&&Number(window.__AGR_FULL_DETAIL_VISIBLE_TARGET__||0)>=15.99;
                    ...
                    return `${mask}|${zoomLabel?.textContent}|${window.__AGR_WHEEL_COUNT__||0}|${window.__AGR_ZOOM_TARGET__||0}|${document.querySelectorAll('.smooth-compare').length}|${document.querySelectorAll('.smooth-compare canvas').length}|${window.__AGR_TEST_VERTICAL_Y__||0}|${window.__AGR_ZOOM_ANCHOR_ERROR__||0}|${window.__AGR_FULL_DETAIL_READY__||0}|${window.__AGR_FULL_AT_800__||false}|${window.__AGR_FULL_DETAIL_SCALE__||0}|${window.__AGR_FULL_DETAIL_VISIBLE_SCALE__||0}|${window.__AGR_FULL_DETAIL_VISIBLE_TARGET__||0}`;
            ...
            QTimer::singleShot(14000,view,[view]{view->page()->runJavaScript(QStringLiteral("document.querySelector('.right-dock [data-route=\"game\"]')?.click()"));});
            QTimer::singleShot(15800,view,[view]{
                view->page()->runJavaScript(QStringLiteral(R"JS((()=>{
